...
def visualize_trajectory_2d(ax, pose, landmarks, path_name="Path", show_ori=False):
    '''
    function to visualize the trajectory in 2D
    '''
    n_pose = pose.shape[2]
    ax.plot(pose[0,3,:],pose[1,3,:],'r-',label=path_name)
    ax.scatter(pose[0,3,0],pose[1,3,0],marker='s',label="start", s=100)
    ax.scatter(pose[0,3,-1],pose[1,3,-1],marker='o',label="end", s=100)
